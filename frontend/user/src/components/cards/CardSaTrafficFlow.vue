<template>
  <div class="card-sa card-sa-traffic-flow">
    <div class="card-sa-row3-header">
      <img src="/page1/Group71.png" alt="" class="card-sa-row3-header-bg" />
      <span class="card-sa-row3-header-title">车流量</span>
    </div>
    <div class="card-sa-body">
      <div ref="chartRef" class="sa-chart-traffic"></div>
    </div>
  </div>
</template>
...
function renderChart() {
  ...
  const seriesData = reversed.map((i) => i.trafficFlow ?? 0)
  chart.setOption({
    tooltip: {
      trigger: 'axis',
