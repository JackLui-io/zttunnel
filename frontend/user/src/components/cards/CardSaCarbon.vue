<template>
  <div class="card-sa card-sa-carbon">
    <div class="card-sa-row3-header">
      <img src="/page1/Group71.png" alt="" class="card-sa-row3-header-bg" />
      <span class="card-sa-row3-header-title">碳排放量与理论碳减排量</span>
    </div>
    <div class="card-sa-body">
      <div ref="chartRef" class="sa-chart-carbon"></div>
...
import type { CarbonByStatisticsVo } from '@/api/analyze'

const props = defineProps<{
  data: CarbonByStatisticsVo[]
}>()
...
function renderChart() {
  ...
  const actualData = reversed.map((i) => i.actualCarbonEmission ?? 0)
  const originalData = reversed.map((i) => i.originalCarbonEmission ?? 0)
  chart.setOption({
    tooltip: {
      trigger: 'axis',
