<template>
  <div class="card-sa card-sa-brightness">
    <div class="card-sa-row3-header">
      <img src="/page1/Group71.png" alt="" class="card-sa-row3-header-bg" />
      <span class="card-sa-row3-header-title">亮度对比</span>
    </div>
    <div class="card-sa-body">
      <div ref="chartRef" class="sa-chart-brightness"></div>
    </div>
  </div>
</template>
...
function renderChart() {
  ...
  const avgOutsideData = reversed.map((i) => i.avgOutside ?? 0)
  const avgDimmingData = reversed.map((i) => i.avgDimmingRadio ?? 0)
  chart.setOption({
    tooltip: {
      trigger: 'axis',
        ...
        areaStyle: {
          color: {
            ...
            colorStops: [
              ...
              { offset: 1, color: 'rgba(222, 240, 214, 0.2)' },
            ],
          },
        },
